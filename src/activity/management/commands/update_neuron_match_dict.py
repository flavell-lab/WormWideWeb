...
def generate_match_dict():
    ...
    neurons = (
        GCaMPNeuron.objects.select_related("neuron_class", "dataset")
        .filter(neuron_class__isnull=False)
        .values(
    ...
    for neuron in neurons:
        ...
        # Dictionary 1
        class_to_combinations[class_name].add(
            tuple([dv, lr])
        )  # Use tuple for set uniqueness
    ...
    neuropal_datasets = GCaMPDataset.objects.filter(
        dataset_type__type_id="common-neuropal"
    )
    ...
    for dataset in neuropal_datasets:
        neuropal_datasets_data.append(
            {
                "paper": {
                    ...
                    "title": dataset.paper.title_short,
                },
        ...
        if not any([dataset.paper.paper_id == paper for paper in neuropal_papers]):
            neuropal_papers.append(
                {"paper_id": dataset.paper.paper_id, "title": dataset.paper.title_short}
            )

        for dtype in dataset.dataset_type.all():
            if dtype.type_id not in neuropal_dataset_type:
                neuropal_dataset_type[dtype.type_id] = {
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        data = {
            "class": dict_class,
            "match": dict_match,
            "neuropal_datasets_data": neuropal_datasets_data,
            "papers": papers,
            "neuropal_dataset_type": neuropal_dataset_type,
        }
