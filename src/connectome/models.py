...
class Neuron(models.Model):
    ...
    name = models.CharField(max_length=50, unique=True)
    neuron_class = models.ForeignKey(
        NeuronClass, on_delete=models.CASCADE, related_name="neurons"
    )
    LR_CHOICES = (("l", "left"), ("r", "right"))
    DV_CHOICES = (("d", "dorsal"), ("v", "ventral"))
...
class Dataset(models.Model):
    ...
    dataset_sha256 = models.CharField(
...
class Synapse(models.Model):
    def __str__(self) -> str:
        return (
            ...
            + self.post.name
            + " n="
            + str(self.synapse_count)
            + " type: "
            + self.synapse_type
        )

    SYNAPSE_CHOICES = (("e", "electrical"), ("c", "chemical"))

    synapse_count = models.PositiveSmallIntegerField()
    synapse_type = models.CharField(
        max_length=1, choices=SYNAPSE_CHOICES, db_index=True
    )
    post = models.ForeignKey(
        Neuron, on_delete=models.CASCADE, related_name="post", db_index=True
    )
    pre = models.ForeignKey(
        Neuron, on_delete=models.CASCADE, related_name="pre", db_index=True
    )
    dataset = models.ForeignKey(
        Dataset, on_delete=models.CASCADE, related_name="synapses", db_index=True
    )
