...
export class ConnectomeGraph {
    constructor(graphId) {
        ...
        this.layoutManager = new GraphLayoutManager(this.graph, null, "dropdownLayout", this.nodePositiobManager, "concentric");
        this.edgeWidthScalingFactor = 1.5
        this.initLayoutSlider();

        // set up edge count threshold
        this.thresholdE = getLocalInt("connectome_threshold_e", 1);
        this.thresholdC = getLocalInt("connectome_threshold_c", 1);
        this.initEdgeCountFilter();

        // set up switches
        this.switchShowIndividual = {value: getLocalBool("connectome_show_individual_neuron", false)}
        this.switchShowConnected = {value: getLocalBool("connectome_show_connected_neuron", true)}
        ...
        initSwitch("switchConnected",
            () => this.debouncedUpdateGraph(),
            () => this.debouncedUpdateGraph(),
            this.switchShowConnected, "connectome_show_connected_neuron",
...
`
    ...
    initEdgeCountFilter() {
        ...
        const setupEdgeFilter = (type) => {
            ...
            plusButton.addEventListener('click', () => {
                ...
                inputField.value = this[`threshold${type.toUpperCase()}`];
                setLocalInt(`connectome_threshold_${type}`, inputField.value)
                this.filterEdge();
            });
    ...
    initLayoutSlider() {
        ...
        const savedSpacing = initSlider("sliderSpacing", null, "connectome_spacing", 1.0, updateSpacing)
        ...
        const savedEdgeScale = initSlider("sliderEdgeScale", null,
            "connectome_edge_scale", 1.5, updateEdgeScalingFactor)
